...
void getNonTerminals(int row){
    ...
    cout<<"Non-Terminals: "<<nonTerminals<<endl<<endl;
}
...
void closure(int index){
    ...
    for(i = 0; i < state[index].production.size(); i++){
        //cout<<"Index: "<<getDotIndex(state[index].production[i])<<endl;
        nextSymbol = state[index].production[i][getDotIndex(state[index].production[i]) + 1];
        if(nextSymbol != 0)
            state[index].input += nextSymbol;
        //cout<<"Production: "<<state[index].production[i]<<" Symbol: "<<nextSymbol<<endl;
...
void removeIfRepeatedState(states &current, int index){
    ...
    cout<<"Current Production size "<<current.production.size()<<endl;
    for(i = 0; i < index; i++){
        if(i != index){
            if(current.production.size() != state[i].production.size()){
                //cout<<i<<" Unequal sizes"<<endl;
                continue;
            }
            for(j = 0; j < current.production.size(); j++){
                //cout<<"Comparing....."<<current.production[j]<<" and "<<state[i].production[j]<<endl;
                if(current.production[j].compare(state[i].production[j]))
                    break;
            }
            //cout<<j<<" "<<current.production.size()<<endl;
            if(j == current.production.size()){
                cout<<i<<" Equal states "<<index<<endl;
...
void gotoState(int index, char transitioSymbol){
    ...
    states *current = new states;
    //cout<<transitioSymbol<<" : "<<occurrence<<endl;
    for(i = 0; i < occurrence; i++){
        pushElement = selectAndTransformProduction(index, transitioSymbol, i);
        //cout<<i<<" Production: "<<pushElement<<endl;
        current->production.push_back(pushElement);
    }
    current->ipPointer = 0;
    //cout<<"Here"<<endl;
    //flagIsRepeated = removeIfRepeatedState(*current);
    //cout<<flagIsRepeated<<endl;
    //if(flagIsRepeated){
    state.push_back(*current);
...
void generateStates(){
    ...
    for(i = 0; i < state.size(); i++)
        for(j = 0; j < state[i].input.length(); j++){
            cout<<"Transition: State "<<i<<"->"<<state[i].input[state[i].ipPointer]<<endl;
...
int main(){
    ...
    printStatesProduction();
    for(i = 0; i < state.size(); i++)
        removeIfRepeatedState(state[i], i);
    printStatesProduction();
    removeIfRepeatedState(state[state.size()],state.size() - 1);
    cout<<state[state.size()].production[0]<<endl;
